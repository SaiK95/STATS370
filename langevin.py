...
for step in range(num_steps):
    ...
    proposal = curr_theta + np.multiply(np.sqrt(step_size_delta), Z) + np.multiply(step_size_delta, gradient)
    ...
    if proposal[0] >= 0 and 0 <= proposal[1] <= 1 and 0 <= proposal[2] <= 1:
        ...
        # `-` instead of `/` because we're operating in log scale
...
# # plot things
f, (ax1, ax2, ax3, ax4, ax5, ax6, ax7, ax8) = plt.subplots(8, 1, figsize=(8, 11))
plt.tight_layout()
